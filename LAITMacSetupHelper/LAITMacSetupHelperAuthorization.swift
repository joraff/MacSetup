...
class LAITMacSetupHelperAuthorization: NSObject {
    ...
    func commandInfo() -> Dictionary<String, Any> {
        ...
        let sCommandInfo: [String:Dictionary<String,Any>] = [
            NSStringFromSelector(#selector(LAITMacSetupHelperProtocol.renameComputer(computerName:authData:reply:))) :
                [AuthorizationRightKey.rightName : "edu.tamu.liberalarts.LAITMacSetupHelper", AuthorizationRightKey.rightDefaultRule : ruleAdminRightsExtended, AuthorizationRightKey.rightDescription : ""],
            NSStringFromSelector(#selector(LAITMacSetupHelperProtocol.adLeave(username:password:authData:reply:))) :
                [AuthorizationRightKey.rightName : "edu.tamu.liberalarts.LAITMacSetupHelper", AuthorizationRightKey.rightDefaultRule : ruleAdminRightsExtended, AuthorizationRightKey.rightDescription : ""],
            NSStringFromSelector(#selector(LAITMacSetupHelperProtocol.adJoin(domain:username:password:computername:authData:reply:))) :
                [AuthorizationRightKey.rightName : "edu.tamu.liberalarts.LAITMacSetupHelper", AuthorizationRightKey.rightDefaultRule : ruleAdminRightsExtended, AuthorizationRightKey.rightDescription : ""],
            NSStringFromSelector(#selector(LAITMacSetupHelperProtocol.adConfigure(groups:authData:reply:))) :
                [AuthorizationRightKey.rightName : "edu.tamu.liberalarts.LAITMacSetupHelper", AuthorizationRightKey.rightDefaultRule : ruleAdminRightsExtended, AuthorizationRightKey.rightDescription : ""]
        ]
        return sCommandInfo
    }
    ...
    func authDataToAuthRef(authData: NSData?) -> AuthorizationRef? {
        var authRef:AuthorizationRef?
        ...
        // Extract the AuthorizationRef from it's external form
        guard AuthorizationCreateFromExternalForm(&authRefExtForm, &authRef) == errAuthorizationSuccess else {
            return nil
        }
        return authRef
    }
